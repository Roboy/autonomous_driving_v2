...
"""
.. module:: calculateHasArrivedFlag
    :platform: ROS
    :synopsis: module calculates and publishes the hasArrivedFlag

.. moduleauthor:: Maximilian Kempa <maximilian.kempa@tum.de>
"""

class CalculateHasArrivedFlag:
    """Class for calculating and publishing hasArrivedFlag

    This class provides some basic methods for calculating and publishing the hasArrivedFlag.
    ...
    def __init__(self, maximum_distance_):
        ...
        Args:
            maximum_distance_ (float): maximum distance from goal position in meters that is still
            acceptable for publishing the hasArrived flag
        """
        self.goal_x = 0
        self.goal_y = 0
    ...
    def start(self):
        """Method to create the subscribers to the ego velocity and to the goal position.
    ...
    def handle_goal_position(self, goal_pos):
        ...
        Args:
            goal_pos (MoveBaseActionGoal): contains the translation and orientation of the goal position
        """
        self.OnlyOneOutputPerGoal = True
    ...
    def euclidDistance(self, goal_pos_x, goal_pos_y, current_pos_x, current_pos_y):
        """Method to calculate the euclidean distance between the current position and the goal position

        Args:
            goal_pos_x (float): x coordinate of goal position in meters
            goal_pos_y (float): y coordinate of goal position in meters
            current_pos_x(float): x coordinate of current position in meters
            current_pos_y(float): y coordinate of current position in meters
        """
        return np.sqrt((goal_pos_x - current_pos_x)**2 + (goal_pos_y - current_pos_y)**2)

    def handle_ego_velocity(self, cmd_vel):
        """Callback for receiving the ego velocity

        Args:
            cmd_vel (float): ego velocity
        """
        ...
        if self.ego_velocity <= self.maximum_velocity and euclid_dist <= self.maximum_distance and self.OnlyOneOutputPerGoal:
